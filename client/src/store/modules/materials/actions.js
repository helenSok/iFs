...
export default {
  async getMaterials({ commit, dispatch, rootState }) {
    let setting = {}
    let exercise_setting = rootState.exercises.current_exercise.exercise_settings.find((t) => t.exercise_type_id == rootState.exercises.exercise_type_id)
  ...
  async getParameters({ commit }, material_id) {
    ...
    let right_answer = parameters.data.map((t) => t).filter((f) => material_parameters.map((e) => e.parameter_id).includes(f.id))
    let parameter_material_character = parameters.data.map((t) => t).filter((f) => right_answer.map((e) => e.parameter_id).includes(f.id))
    let parameter_material_head = parameters.data.map((t) => t).filter((f) => parameter_material_character.map((e) => e.parameter_id).includes(f.id))
    let parameter_material_description = parameters.data.map((t) => t).filter((f) => parameter_material_character.map((e) => e.id).includes(f.parameter_id))
